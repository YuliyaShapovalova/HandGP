...
drug_pair = 'LA_antagonism'
drug_name = 'LA_antagonism'
...
plt.savefig('figures/LA_antagonism/'+drug_name+'_DrugA_2drugs'+'.png')
...
plt.ylabel('Response', fontsize=20)
plt.tick_params(axis='both', which='major', labelsize=20)
plt.savefig('figures/LA_antagonism/'+drug_name+'_DrugB_2drugs'+'.png')
...
plt.savefig('figures/LA_antagonism/'+str(drug_name)+'_Hand_contour_result'+'.png', bbox_inches = 'tight',
    pad_inches = 0)
fig, ax = plt.subplots(figsize=(6,6))
#ax.set_aspect('equal')
...
plt.savefig('figures/LA_antagonism/'+str(drug_name)+'_GP_mean'+'.png', bbox_inches = 'tight',
    pad_inches = 0)
...
plt.savefig('figures/LA_antagonism/'+str(drug_name)+'_GP_residuals'+'.png')

df1 = pd.read_csv('../data/TestData2LA_antagonism.csv', sep= ",")
df2 = pd.read_csv('../data/TestDataLA_antagonism.csv', sep= ",")
...
df1 = pd.DataFrame(data=d1)
df1.columns = ['fa', 'fb', 'xx_a', 'xx_b']
df1.to_csv('TestData'+str(drug_name)+'.csv')

d2 = np.concatenate((X1, X2, Dose_A, Dose_B), axis=1)
#d2 = {'X1': [X1], 'X2': [X2], 'Dose_A': [Dose_A], 'Dose_B': [Dose_B]}
df2 = pd.DataFrame(data=d2)
df2.columns = ['X1', 'X2', 'Dose_A', 'Dose_B']
df2.to_csv('TestData2'+str(drug_name)+'.csv')


df1 = pd.read_csv('TestData2LA_antagonism.csv', sep= ",")
df2 = pd.read_csv('TestDataLA_antagonism.csv', sep= ",")
...
for t in cbar.ax.get_yticklabels():
    t.set_fontsize(15)
plt.savefig('figures/LA_antagonism/'+drug_name+'contour_result'+'.png',bbox_inches = 'tight',
    pad_inches = 0)
...
plt.xlabel('log($x_1$/$l_1$+1)', fontsize=15)
plt.ylabel('log($x_2$/$l_2$+1)', fontsize=15)
plt.savefig('figures/LA_antagonism/'+drug_name+'_mean_full_contour_result'+'.png',bbox_inches = 'tight',
    pad_inches = 0)
...
plt.xlabel('log($x_1$/$l_1$+1)', fontsize=15)
plt.ylabel('log($x_2$/$l_2$+1)', fontsize=15)
plt.savefig('figures/LA_antagonism/'+drug_name+'_Hand_contour_result'+'.png',bbox_inches = 'tight',
    pad_inches = 0)
...
plt.xlabel('log($x_1$/$l_1$+1)', fontsize=15)
plt.ylabel('log($x_2$/$l_2$+1)', fontsize=15)
plt.savefig('figures/LA_antagonism/'+drug_name+'_GP_residuals'+'.png',bbox_inches = 'tight',
    pad_inches = 0)
...
def plot_samples(step_size, num_leapfrog, samples, parameters, y_axis_label):
    ...
    plt.savefig('figures/LA_antagonism/LA_antagonism_samples_'+str(step_size)+str(num_leapfrog)+'.png')
...
def marginal_samples(step_size, leap_frog, samples, parameters, y_axis_label):
    ...
    for ax, val, param in zip(axes, samples, parameters):
        ...
        ax.set_title(param_to_name[param])
    fig.suptitle(y_axis_label)
    plt.savefig('figures/LA_antagonism/LA_antagonism_samples_'+str(step_size)+str(leap_frog)+'.png')
    #plt.show()
    ...
    print(df)
    df = df.round(2)
    df.to_csv('LA_antagonism_hyperparameters'+str(step_size)+str(leap_frog)+'.csv')

#leapfrog_num_and_step_size = np.stack(np.meshgrid([20.0, 15.0, 10.0, 5.0, 1.0], [0.1, 0.5, 0.7, 0.9, 1.0, 1.5, 2.0])).T.reshape(-1, 2)
leapfrog_num_and_step_size = np.stack(np.meshgrid([ 5.0], [0.15])).T.reshape(-1, 2)
