...
from utilities import (trapezoidal_area, compute_prior_hyperparameters, trapezoidal_area, fit_1d_model, predict_in_observations, fit_Hand,y_exp_Hand1,  y_exp_Hand, y_exp, fit_3d_model, find_nearest, K_log, K_multiplicative)
...
#mean_full = pd.read_csv('LA_bidirectional_synergy_mean_full_new.csv', sep=',', header=None).to_numpy()
...
print('init_variance', init_var)
print('minimum likelihood', np.nanmin(Lik_full))
...
#m.kernel.lengthscale_da.assign(9.888)
#m.kernel.lengthscale_db.assign(9.888)
#m.kernel.variance_da.assign(33.)
...
#plt.title("Hand-GP estimated effect", fontsize=20)
plt.savefig(drug_name+'contour_result'+'.png', bbox_inches = 'tight',
    pad_inches = 0)
...
plt.ylabel('log($x_2$/$l_2$+1)', fontsize=15)
#plt.title("GP estimated surface", fontsize=20)
plt.savefig(drug_name+'_mean_full_contour_result'+'.png', bbox_inches = 'tight',
    pad_inches = 0)
...
plt.savefig(drug_name+'_Hand_contour_result'+'.png', bbox_inches = 'tight',
    pad_inches = 0)
...
plt.ylabel('log($x_2$/$l_2$+1)', fontsize=15)
#plt.title("GP residuals", fontsize=20)
plt.savefig(drug_name+'_GP_residuals'+'.png', bbox_inches = 'tight',
    pad_inches = 0)
#exit()
# fig, ax = plt.subplots(figsize=(6,6))
# ax.set_aspect('equal')
...
def plot_samples(step_size, num_leapfrog, samples, parameters, y_axis_label):
    ...
    plt.savefig('LA_synergy_samples_'+str(step_size)+str(num_leapfrog)+'.png')
...
def marginal_samples(step_size, leap_frog, samples, parameters, y_axis_label):
    ...
    for ax, val, param in zip(axes, samples, parameters):
        ...
        ax.set_title(param_to_name[param])
    fig.suptitle(y_axis_label)
    plt.savefig('LA_synergy_samples_'+str(step_size)+str(leap_frog)+'.png')
    #plt.show()
...
#leapfrog_num_and_step_size = np.stack(np.meshgrid([ 5.0], [0.1, 0.15, 0.2])).T.reshape(-1, 2)
leapfrog_num_and_step_size = np.stack(np.meshgrid([ 5.0], [ 0.2])).T.reshape(-1, 2)
