...
mean_full = pd.read_csv('LA_bidirectional_synergy_mean_full.csv', sep=',', header=None).to_numpy()
...
alphaA, betaA = compute_prior_hyperparameters(A_max/3, 0.1*A_max)
alphaB, betaB = compute_prior_hyperparameters(B_max/3, 0.1*B_max)
...
df1.to_csv('TestData'+str(drug_pair)+'.csv')
...
df2.to_csv('TestData2'+str(drug_pair)+'.csv')
############################################################################
...
ax.set_aspect('equal')
#cf = ax.contourf(Dose_A.flatten(), Dose_B.flatten(), mean_full)
cf = ax.contourf(np.log(Dose_A.flatten()/np.array(m.kernel.lengthscale_da.value())+1), np.log(Dose_B.flatten()/np.array(m.kernel.lengthscale_da.value()+1)), mean_full)
...
fig.subplots_adjust(left=0.15, bottom=0.15, right=0.95, top=0.8)
#cf = ax.contourf(Dose_A.flatten(), Dose_B.flatten(),Y_expected_Hand,v)
cf = ax.contourf(np.log(Dose_A.flatten()/np.array(m.kernel.lengthscale_da.value())+1), np.log(Dose_B.flatten()/np.array(m.kernel.lengthscale_da.value())+1),Y_expected_Hand,v)
cbar = fig.colorbar(cf, ax=ax)
