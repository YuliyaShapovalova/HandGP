...
print(df1)
df1 = pd.read_csv('TestData2LA_bidirectional_antagonism_new.csv', sep= ",")
df2 = pd.read_csv('TestDataLA_bidirectional_antagonism_new.csv', sep= ",")
...
def plot_samples(step_size, num_leapfrog, samples, parameters, y_axis_label):
    ...
    plt.savefig('LA_antagonism_samples_'+str(step_size)+str(num_leapfrog)+'.png')
...
def marginal_samples(step_size, leap_frog, samples, parameters, y_axis_label):
    ...
    for ax, val, param in zip(axes, samples, parameters):
        ...
        ax.set_title(param_to_name[param])
    fig.suptitle(y_axis_label)
    plt.savefig('LA_antagonism_samples_'+str(step_size)+str(leap_frog)+'.png')
    #plt.show()
