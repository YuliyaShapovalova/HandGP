...
df = pd.read_csv('YonetaniData2.csv', sep=";")
#df = df.iloc[1:]
drug_name = 'Yonetani_2'
...
Dose_B = df['Conc.Phen'].values.astype(float).copy()
...
Effect_A = df[df['Conc.Phen'] == 0]['FracInhib'].to_numpy().reshape(-1,1).astype(float)
Dose_A = df[df['Conc.Phen']==0]['Conc.ADP'].to_numpy().reshape(-1,1).astype(float)
...
#data_save.to_csv('Yonetani2_weighted.csv')
...
cf = ax.contourf(np.log(Dose_A.flatten()+np.array(m.kernel.lengthscale_da.value())), np.log(Dose_B.flatten()+np.array(m.kernel.lengthscale_db.value())),Y_expected_Hand,v)
...
#ax.axis('equal')
#ax.set_aspect('equal')
cf = ax.contourf(np.log(Dose_A.flatten()+np.array(m.kernel.lengthscale_da.value())), np.log(Dose_B.flatten()+np.array(m.kernel.lengthscale_db.value())),mean_full,v)

#ax.set_xlim(right=np.max(np.log(Dose_A.flatten()+np.array(m.kernel.lengthscale_da.value()))))
#ax.set_ylim(top=np.max(np.log(Dose_B.flatten()+np.array(m.kernel.lengthscale_db.value()))))

print('Dose_A', np.log(Dose_A.flatten()+np.array(m.kernel.lengthscale_da.value())))
print('Dose_B', np.log(Dose_B.flatten()+np.array(m.kernel.lengthscale_db.value())))
cbar = fig.colorbar(cf, ax=ax)
...
# plt.xlabel('log(dose_a +l_a)', fontsize=20)
# plt.ylabel('log(dose_b +l_b)', fontsize=20)
# plt.title('Difference between GP and Hand_GP models', fontsize=20)
...
cf = ax.contourf(np.log(Dose_A.flatten()+np.array(m.kernel.lengthscale_da.value())), np.log(Dose_B.flatten()+np.array(m.kernel.lengthscale_db.value())),mean_full - Effect.reshape(6,6),v, cmap='YlGnBu')
...
plt.savefig(str(drug_name)+'_GP_residuals'+'.png', bbox_inches = 'tight',
    pad_inches = 0)
...
plt.tick_params(axis='both', which='major', labelsize=20)
plt.savefig('ChouTalalay2_GP'+'_diagonal'+'.png')
...
plt.savefig('ChouTalalay2_GP'+'_diagonal_null'+'.png', bbox_inches = 'tight',
    pad_inches = 0)
...
plt.tick_params(axis='both', which='major', labelsize=20)
plt.savefig('ChouTalalay2_GP'+'_diagonal_null_difference'+'.png', bbox_inches = 'tight',
    pad_inches = 0)
...
#     plt.xlabel("HMC iteration")
#     plt.ylabel(y_axis_label)
#     plt.savefig('ChouTalalay2_samples'+str(step_size)+str(num_leapfrog)+'.png')
...
def marginal_samples(step_size,leap_frog, samples, parameters, y_axis_label):
    ...
    for ax, val, param in zip(axes, samples, parameters):
        ...
        ax.set_title(param_to_name[param])
    fig.suptitle(y_axis_label)
    plt.savefig('ChouTalalay2_marginal_samples_'+str(step_size)+str(num_leapfrog)+'.png')
    #plt.show()
    res = np.zeros((4, 2))
    for i in range(3):
    ...
    print(df)
    df = df.round(5)
    df.to_csv('ChouTalalay2_hyperparameters'+str(step_size)+str(leap_frog)+'.csv')

#leapfrog_num_and_step_size = np.stack(np.meshgrid([1.0, 5.0, 10.0, 15.0, 20.0], [0.001, 0.005, 0.01, 0.05])).T.reshape(-1, 2)
leapfrog_num_and_step_size = np.stack(np.meshgrid([5.0], [0.05])).T.reshape(-1, 2)
