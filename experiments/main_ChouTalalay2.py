...
plt.savefig('figures/'+drug_name+'_DrugA'+'.png')
...
plt.ylim(0.0, 1.1)
plt.tick_params(axis='both', which='major', labelsize=20)
plt.savefig('figures/'+drug_name+'_DrugB'+'.png')
...
def marginal_samples(step_size,leap_frog, samples, parameters, y_axis_label):
    ...
    print(df)
    df = df.round(5)
    df.to_csv('results/ChouTalalay2_hyperparameters'+str(step_size)+str(leap_frog)+'.csv')
