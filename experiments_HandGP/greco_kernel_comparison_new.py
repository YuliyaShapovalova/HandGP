...
[l_init, var_init] = np.meshgrid(np.linspace(0.01, 10.0, 10), np.linspace(0.001, 0.1, 10))
...
for i in range(1,100):
    try:
        init_lengthscale = l_init[i,0]
        init_variance = var_init[i,0]
...
init_lengthscale = l_init[index,0]
init_variance = var_init[index,0]
...
plt.xlabel('$x_1$/l_1+1', fontsize=20)
...
## plot
plt.figure(figsize=(12, 6))
plt.plot(np.log(Dose_A/m_rbf.kernel.kernels[0].lengthscales.value()+1), Effect_A, "kx", mew=2)
plt.plot(np.log(xx_A/m_rbf.kernel.kernels[0].lengthscales.value()+1), mean_full_est.loc[0],"C0", lw=2, color='purple')
plt.fill_between(
    np.log(xx_A[:, 0]/m_rbf.kernel.kernels[0].lengthscales.value()+1),
    mean_full_est.loc[0] - 1.96 * np.sqrt(Cov_full_est.loc[0]),
    mean_full_est.loc[0] + 1.96 * np.sqrt( Cov_full_est.loc[0]),
    color="purple",
    alpha=0.2
...
plt.tick_params(axis='both', which='major', labelsize=15)
#plt.title('Dose-response curve')
plt.savefig('Greco'+'DrugA_in_log_rbf'+'.png')


## plot
plt.figure(figsize=(12, 6))
plt.plot(np.log(Dose_B/m_rbf.kernel.kernels[1].lengthscales.value()+1), Effect_B, "kx", mew=2)
plt.plot(np.log(xx_B/m_rbf.kernel.kernels[1].lengthscales.value()+1), mean_full_est.iloc[:,0],"C0", lw=2, color='purple')
plt.fill_between(
    np.log(xx_B[:, 0]/m_rbf.kernel.kernels[1].lengthscales.value()+1),
    mean_full_est.iloc[:,0] - 1.96 * np.sqrt(Cov_full_est.iloc[:,0]),
    mean_full_est.iloc[:,0] + 1.96 * np.sqrt( Cov_full_est.iloc[:,0]),
    color="purple",
    alpha=0.2
...
plt.xlabel('$log(x_2/l_2+1)$', fontsize=20)
plt.tick_params(axis='both', which='major', labelsize=15)
plt.savefig('Greco'+'DrugB_in_log_rbf'+'.png')
