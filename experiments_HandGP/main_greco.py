...
#drug_pair = 'ABA.WOR' # Not monotonic
#drug_pair = 'BRO.BRO'
#drug_pair = 'LAT.RAD'
#drug_pair = 'CYC.RAD'
...
plt.plot(np.log(xx_A/m_full.kernel.lengthscale_da.value()+1), mean_full_est.loc[0],"C0", lw=2, color='red')
plt.fill_between(
...
plt.plot(np.log(xx_B/m_full.kernel.lengthscale_db.value()+1), mean_full_est.iloc[:,0],"C0", lw=2, color='red')
plt.fill_between(
...
print(Effect.flatten().reshape(6,6))
...
print(df)
print((mean_full.flatten()-Effect.flatten()).reshape(6,6))


fig, ax = plt.subplots(figsize=(6,6))
#ax.set_aspect('equal')
#v = np.linspace(-8, 10, 10, endpoint=True)
v = np.linspace(-16, 16, 10, endpoint=True)
fig.subplots_adjust(left=0.15, bottom=0.15, right=0.95, top=0.8)
cf = ax.contourf(Dose_A.flatten(), Dose_B.flatten(),(mean_full.flatten()-Effect.flatten()).reshape(6,6), v,cmap='YlGnBu')
cbar = fig.colorbar(cf, ax=ax)
...
leapfrog_num_and_step_size = np.stack(np.meshgrid([5.0], [0.7])).T.reshape(-1, 2)
